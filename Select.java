...
public class Select extends JPanel implements MouseListener , MouseMotionListener {
	private Menu m;
	private Game g;
	private Map p;
	...
	public void paintComponent(Graphics g){
	 ...
	 public void mousePressed(MouseEvent e){	
		 ...
		 if( (e.getX()>=100 && e.getX()<=400 )&&(e.getY()>=300 && e.getY()<=600 )  ){
			  ...
			  pf = new JFrame("King of Fighters"); 
			  p = new Map(1,pf);
			
			  
			  pf.add(p);
			  pf.setExtendedState(pf.getExtendedState() | Frame.MAXIMIZED_BOTH);
			  pf.setVisible(true);
		      bgm.stop();
		      Selectframe.dispose();
		      f1.dispose();
